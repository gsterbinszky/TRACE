...
# Synchronize country names between TRACE and NBIM
...
GPFG <- left_join(oljefondet, trace_matrix, by = "Country")

# Convert risk scores to numeric
GPFG <- GPFG %>%
  mutate(`Total Risk score` = as.numeric(GPFG$`Total Risk score`))
...
GPFG <- left_join(df, iso_codes, by = "Country")
...
# Domains
GPFG$`Overall Opportunity risk` <- as.numeric(GPFG$`Overall Opportunity risk`)
GPFG$`Overall Deterrence risk` <- as.numeric(GPFG$`Overall Deterrence risk`)
GPFG$`Overall Transparency risk` <- as.numeric(GPFG$`Overall Transparency risk`)
GPFG$`Overall Oversight risk` <- as.numeric(GPFG$`Overall Oversight risk`)

# Sub-domains
GPFG$Interaction <- as.numeric(GPFG$Interaction)
GPFG$Expectation <- as.numeric(GPFG$Expectation)
GPFG$Leverage <- as.numeric(GPFG$Leverage)
GPFG$Dissuasion <- as.numeric(GPFG$Dissuasion)
GPFG$Enforcement <- as.numeric(GPFG$Enforcement)
GPFG$Processes <- as.numeric(GPFG$Processes)
GPFG$Interests <- as.numeric(GPFG$Interests)
GPFG$`Free press` <- as.numeric(GPFG$`Free press`)
GPFG$`Civil society` <- as.numeric(GPFG$`Civil society`)

# Join with CPI
GPFG <- left_join(GPFG, df_cpi, by = "Country")
GPFG$CPI_score <- as.numeric(GPFG$CPI_score)

# Join with CoC
GPFG <- left_join(GPFG, df_coc, by = "Country")
GPFG$CoC_score <- as.numeric(GPFG$CoC_score)

# Join with GDI
GPFG <- left_join(GPFG, df_gdi, by = "Country")
GPFG$GDI_score <- as.numeric(GPFG$GDI_score)

# Calculate portfolio weighted risk scores
GPFG <- GPFG %>%
...
portfolio_oljefondet <- data.frame(rbind(sum(GPFG$investment_risk_score_1), sum(GPFG$investment_risk_score_2), 
                                         sum(GPFG$investment_risk_score_3), sum(GPFG$investment_risk_score_4), 
                                         sum(GPFG$investment_risk_score_5), sum(GPFG$investment_risk_score_6), 
                                         sum(GPFG$investment_risk_score_7), sum(GPFG$investment_risk_score_8),
                                         sum(GPFG$investment_risk_score_9), sum(GPFG$investment_risk_score_10),
                                         sum(GPFG$investment_risk_score_11), sum(GPFG$investment_risk_score_12),
                                         sum(GPFG$investment_risk_score_13), sum(GPFG$investment_risk_score_14),
                                         sum(GPFG$investment_risk_score_15), sum(GPFG$investment_risk_score_16),
                                         sum(GPFG$investment_risk_score_17)))
# Add rownames
...
# Read in file
NZSUPER <- read_xlsx("nzsf.xlsx", range = "A9:D6649")
...
NZSUPER$Country <- factor(NZSUPER$Country)
NZSUPER$GICSIndustryDescr <- factor(NZSUPER$GICSIndustryDescr)
...
NZSUPER <- left_join(NZSUPER, trace_matrix, by = "Country")

fund_value_zealand <- sum(NZSUPER$`Value in New Zealand Dollars`)

# Domains
NZSUPER$`Overall Opportunity risk` <- as.numeric(NZSUPER$`Overall Opportunity risk`)
NZSUPER$`Overall Deterrence risk` <- as.numeric(NZSUPER$`Overall Deterrence risk`)
NZSUPER$`Overall Transparency risk` <- as.numeric(NZSUPER$`Overall Transparency risk`)
NZSUPER$`Overall Oversight risk` <- as.numeric(NZSUPER$`Overall Oversight risk`)
NZSUPER$`Value in New Zealand Dollars` <- as.numeric(NZSUPER$`Value in New Zealand Dollars`)
NZSUPER$`Total Risk score` <- as.numeric(NZSUPER$`Total Risk score`)

# Sub-domains
NZSUPER$Interaction <- as.numeric(NZSUPER$Interaction)
NZSUPER$Expectation <- as.numeric(NZSUPER$Expectation)
NZSUPER$Leverage <- as.numeric(NZSUPER$Leverage)
NZSUPER$Dissuasion <- as.numeric(NZSUPER$Dissuasion)
NZSUPER$Enforcement <- as.numeric(NZSUPER$Enforcement)
NZSUPER$Processes <- as.numeric(NZSUPER$Processes)
NZSUPER$Interests <- as.numeric(NZSUPER$Interests)
NZSUPER$`Free press` <- as.numeric(NZSUPER$`Free press`)
NZSUPER$`Civil society` <- as.numeric(NZSUPER$`Civil society`)

#CPI
NZSUPER <- left_join(NZSUPER, df_cpi, by = "Country")
NZSUPER$CPI_score <- as.numeric(NZSUPER$CPI_score)

#CoC
NZSUPER <- left_join(NZSUPER, df_coc, by = "Country")
NZSUPER$CoC_score <- as.numeric(NZSUPER$CoC_score)

#GDI
NZSUPER <- left_join(NZSUPER, df_gdi, by = "Country")
NZSUPER$GDI_score <- as.numeric(NZSUPER$GDI_score)
...
# Calculate portfolio scores
portfolio_zealand <- data.frame(rbind(sum(NZSUPER$investment_risk_score_1), 
                                      sum(NZSUPER$investment_risk_score_2),
                                      sum(NZSUPER$investment_risk_score_3), 
                                      sum(NZSUPER$investment_risk_score_4),
                                      sum(NZSUPER$investment_risk_score_5), 
                                      sum(NZSUPER$investment_risk_score_6, na.rm = TRUE),
                                      sum(NZSUPER$investment_risk_score_7, na.rm = TRUE), 
                                      sum(NZSUPER$investment_risk_score_8, na.rm = TRUE),
                                      sum(NZSUPER$investment_risk_score_9, na.rm = TRUE), 
                                      sum(NZSUPER$investment_risk_score_10, na.rm = TRUE),
                                      sum(NZSUPER$investment_risk_score_11, na.rm = TRUE), 
                                      sum(NZSUPER$investment_risk_score_12, na.rm = TRUE),
                                      sum(NZSUPER$investment_risk_score_13, na.rm = TRUE), 
                                      sum(NZSUPER$investment_risk_score_14, na.rm = TRUE),
                                      sum(NZSUPER$investment_risk_score_15, na.rm = TRUE),
                                      sum(NZSUPER$investment_risk_score_16, na.rm = TRUE),
                                      sum(NZSUPER$investment_risk_score_17, na.rm = TRUE)))
...
GPFG$risk_cat <- factor(GPFG$risk_cat)

# Calculate category shares
oljefondet_category_shares <- GPFG %>%
  group_by(risk_cat) %>%
  select(Name, `Market Value(USD)`) %>%
  summarise(n = n(),
            share = round(n / nrow(GPFG), 3),
...
# NZ Super Fund
###############
NZSUPER$risk_cat <- factor(NZSUPER$risk_cat)

# Calculate category shares
zealand_category_shares <- NZSUPER %>%
  group_by(risk_cat) %>%
  select(`Security Name`, `Value in New Zealand Dollars`) %>%
  summarise(n = n(),
            share = round(n / nrow(NZSUPER), 3),
...
plot5 <- rbind(oljefondet_plot5, apfc_plot5, nzsuper_plot5, abp_plot5)

plot5$Country <- factor(plot5$Country)
plot5$scope <- factor(plot5$scope, levels = c("GPFG", "ABP", "APFC", "NZ Super"))
